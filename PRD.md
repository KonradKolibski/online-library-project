...
- **AI natural-language search in Discover.** Alongside the bubble-tree interface, offer a personalized, AI-powered search where the user types a request in plain language — *"books like Piranesi but shorter," "uplifting non-fiction about cities," "something to read after a breakup," "a fast sci-fi I haven't read yet."* The agent grounds its suggestions in the user's own data: the books they've added to their library, what they've finished vs. abandoned, their ratings, categories/tags, and (once it ships) reading-session and mood-log history. This is the *personal* counterpart to the bubbles' *curated/exploratory* layer — bubbles capture explicit, in-the-moment intent; AI search reads the implicit patterns across the user's reading life. Results surface as the same swipeable recommendation cards (swipe right to "Want to read," left to dismiss), so the two entry points share one downstream experience.
  - *Approach to keep cost/latency sane:* maintain a precomputed per-user "taste profile" (updated when a book is added, finished, or rated) rather than feeding the whole library to the model on every query, and reserve live model calls for interpreting the natural-language query and ranking candidates.
  - *Scope note:* start with natural-language search alone as the first AI slice; weekly personalized picks, auto-themed smart shelves, and a reading coach are natural follow-ons that can reuse the same taste-profile backend.
